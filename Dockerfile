...
RUN go build -a -ldflags="-w -s \
    -X github.com/voidcontests/coyote/internal/version.Commit=$(git rev-parse --short HEAD) \
    -X github.com/voidcontests/coyote/internal/version.Branch=$(git rev-parse --abbrev-ref HEAD)" \
